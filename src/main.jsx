    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        try {
          ...
          let width = Math.round(img.width * 2);
          let height = Math.round(img.height * 2);
          canvas.width = width;
  ...
  const processOCR = async () => {
    ...
    try {
      for (let i = 0; i < images.length; i++) {
        const img = images[i];
        setOcrProgress({ current: i + 1, total: images.length, status: `Preparando imagem ${i + 1} de ${images.length}...` });
        try {
          const processedBlob = await preprocessImage(img.file);
          setOcrProgress({ current: i + 1, total: images.length, status: `Lendo imagem ${i + 1} de ${images.length}...` });
  ...
  return (
    <main className="app">
      ...
      <section className="panel">
        ...
        <p className="muted">
          Envie os prints. Os <strong>pontos</strong> são lidos automaticamente.
          Se o nome não for reconhecido, entra como "Jogador 1", "Jogador 2"... e você edita.
        </p>
        ...
        {!isProcessing && rawTexts.length > 0 && (
          <div className="extracted-section">
            {extracted.length > 0 ? (
              <>
                <h3 style={{ color: '#15803d' }}>✓ {extracted.length} registro(s) com pontos — revise os nomes</h3>
                <p className="muted" style={{ marginBottom: 12 }}>
                  Já foram lançados no ranking. Corrija os nomes que aparecerem como "Jogador 1", "???" etc.
                </p>
                ...
                <div className="extracted-actions">
                  <button type="button" className="btn-secondary" onClick={() => setExtracted([])}>Fechar esta lista</button>
                </div>
              </>
            ) : (
            ...
            <details className="raw-text" open={extracted.length === 0}>
              <summary>{extracted.length === 0 ? 'Texto bruto do OCR' : 'Ver texto bruto (debug)'}</summary>
              {rawTexts.map((r, i) => (
      ...
      <section className="panel">
        ...
        <p className="muted">Use para corrigir nomes ou adicionar quem o OCR não pegou.</p>
